...
import { getCourts, deleteCourtById } from 'apiSdk/courts';
import { CourtInterface } from 'interfaces/court';

type ColumnType = ColumnDef<CourtInterface, unknown>;
...
export function CourtListPage(props: CourtListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CourtInterface>>(
    () => `/courts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CourtInterface) => {
    if (hasAccess('court', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/courts/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/courts/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('court', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/courts/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('court', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="court" />
        </Flex>

        {hasAccess('court', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/courts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
