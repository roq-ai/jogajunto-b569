...
function ScheduleCreatePage() {
  ...
  const formik = useFormik<ScheduleInterface>({
    initialValues: {
      ...
      match_time: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="match_time" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Match Time
            </FormLabel>
            <DatePicker
              selected={formik.values?.match_time ? new Date(formik.values?.match_time) : null}
              onChange={(value: Date) => formik.setFieldValue('match_time', value)}
            />
          </FormControl>
