...
import { getCourts } from 'apiSdk/courts';
...
function DashboardPage() {
  ...
  const courtStat = useSWR<PaginatedInterface<CourtInterface>>(
    () => `/courts`,
    () => getCourts({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Courts',
      result: courtStat?.data?.totalCount,
      isLoading: courtStat.isLoading,
