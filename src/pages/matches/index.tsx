...
export function MatchListPage(props: MatchListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getMatches({
        relations: ['user_match_user_idTouser', 'schedule', 'user_match_opponent_idTouser'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('schedule', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/schedules/view/${record.schedule?.id}`}>
              {record.schedule?.match_date}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_match_opponent_idTouser',
          header: 'User Match Opponent Id Touser',
          accessorKey: 'user_match_opponent_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_match_opponent_idTouser?.id}`}
            >
              {record.user_match_opponent_idTouser?.email}
            </Link>
          ),
        }
      : null,
